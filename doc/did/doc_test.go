...
func TestVerifyProof(t *testing.T) {
	...
	for _, d := range docs {
		...
		signedDoc := createSignedDidDocument(t, pubKey)
		...
		err = doc.VerifyProof(
			[]api.VerifierSuite{&signature.MockVerifierSuite{
...
func createSignedDidDocument(t *testing.T, pubKey []byte) []byte {
	const (
	...
	s := signer.New(&signature.MockSignerSuite{
		MockSuite: signature.MockSuite{AcceptVal: true},
		SignVal:   []byte("thirty-two bytes for a signature"),
		AlgVal:    "mock-alg",
	})
